...
class ConservativeSAC(object):
    ...
    def get_default_config(updates=None):
        ...
        config.reds_temp = 1.0

        if updates is not None:
            config.update(ConfigDict(updates).copy_and_resolve_references())
        return config

    def __init__(self, config, policy, qf, mu):
        ...
        mu_params = self.mu.init(
            next_rng(self.mu.rng_keys()),
            jnp.zeros((10, self.observation_dim))
        )
        self._train_states['mu'] = TrainState.create(
            params=mu_params,
            tx=optimizer_class(self.config.policy_lr),
            apply_fn=None
        )
    ...
    def _train_step(self, train_states, target_qf_params, rng, batch, bc=False):
        ...
        def loss_fn(train_params):
            ...
            v1_pred = forward_qf(train_params['qf1'], observations, new_actions)
            v2_pred = forward_qf(train_params['qf2'], observations, new_actions)

            v_pred = jnp.minimum(v1_pred, v2_pred)
            q_pred = (q1_pred + q2_pred) / 2.0
            advantage = q_pred - v_pred
            advantage = advantage.reshape(mu_log_probs.shape)
            
            # use clipped negative advantage
            weight = jnp.clip(-advantage * self.config.reds_temp, a_min=-10, a_max=5)
            mu_loss = -(jnp.exp(weight) * mu_log_probs).mean()
            loss_collection['mu'] = mu_loss
            ...
            if self.config.use_cql:
                ...
                cql_q1_mu_actions = forward_qf(train_params['qf1'], observations, mu_actions)
                cql_q2_mu_actions = forward_qf(train_params['qf2'], observations, mu_actions)
        ...
        metrics = collect_metrics(
            aux_values,
            ['log_pi', 'policy_loss', 'qf1_loss', 'qf2_loss', 'alpha_loss', 'advantage', 'weight', 'mu_loss',
             'alpha', 'q1_pred', 'q2_pred', 'v1_pred', 'v2_pred', 'q_pred', 'v_pred', 'target_q_values', 
             'mu_log_probs']
        )
        ...
        if self.config.use_cql:
            metrics.update(collect_metrics(
                ...
                ['cql_std_q1', 'cql_std_q2', 'cql_q1_rand', 'cql_q2_rand'
                 ...
                 'alpha_prime_loss', 'cql_q1_mu_actions', 'cql_q2_mu_actions',],
                'cql'
            ))
